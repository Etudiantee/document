 ...
 // State to hold success/error messages
  ...
  useEffect(() => {
    ...
    const fetchFlightDetails = async () => {
      try {
        const response = await axios.get(`http://localhost/devtest/reactjs/newfeuille.php?TLC=${TLC}&dayOfOrigin=${formData.DAY_OF_ORIGIN}`);
        if (response.data) {
 ...
 useEffect(() => {
  ...
  const handleInsertData = async () => {
    try {
      ...
      const response = await axios.post('http://localhost/devtest/reactjs/newfeuille.php', updatedTableData);
  ...
  useEffect(() => {
    ...
    const fetchDayOfOriginOptions = async () => {
      try {
        const response = await axios.get(`http://localhost/devtest/reactjs/getDayOfOrigin.php?tlc=${TLC}`);
...
<label class="info">Choose the date to fill in :</label>
...
>
  {[...Array(100)].map((_, i) => (
    <option key={i + 1} value={i + 1}>{i + 1}</option>
          ...
          <button onClick={handleInsertData} className="btn btn-primary mt-3" style={{marginLeft:'1300px',backgroundColor:'#365D86'}}>CONTINUE</button>
        </div>
      </div>
    </div>
  );
